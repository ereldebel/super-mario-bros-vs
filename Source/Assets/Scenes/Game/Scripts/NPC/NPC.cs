...
namespace Scenes.Game.Scripts.NPC
{
	...
	public class NPC : MonoBehaviour, IHittable
	{
		...
		public virtual bool TakeHit(Vector2 normal)
		{
		...
		/// <summary>
		/// If exited the camera frame form the bottom - destroys self.
		/// </summary>
		private void DestroySelfIfNeeded()
		{
			if (transform.position.y < GameManager.CameraPosition.y - 5)
				Destroy(gameObject);
		}
